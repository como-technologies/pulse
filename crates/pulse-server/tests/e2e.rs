...
async fn full_http_flow() {
    ...
    assert_eq!(questions[0]["question_batch_id"], batch_id.0.to_string());
...
async fn questions_include_segment_vector() {
    ...
    let q = &questions[0];
    assert_eq!(q["question_batch_id"], batch_id.0.to_string());
...
async fn sign_denied_frequency_cap() {
    ...
    let blinding1 = blind_sig::blind(&pk, &token_bytes1.0).unwrap();
    ...
    let blinding2 = blind_sig::blind(&pk, &token_bytes2.0).unwrap();
