...
impl ResponseStore for SqliteStore {
    fn store(&self, response: StoredResponse) {
        ...
        conn.execute(
            "INSERT INTO responses (encrypted_blob, question_batch_id, received_at) VALUES (?1, ?2, ?3)",
            rusqlite::params![
    ...
    fn list(&self) -> Vec<StoredResponse> {
        ...
        let mut stmt = conn
            .prepare(
                "SELECT encrypted_blob, question_batch_id, received_at FROM responses ORDER BY id",
            )
            .expect("response store db error");
        stmt.query_map([], |row| {
            let blob: Vec<u8> = row.get(0)?;
            let batch_id_str: String = row.get(1)?;
            let received_at: u64 = row.get(2)?;
            Ok(StoredResponse {
